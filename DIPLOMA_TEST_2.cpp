...
// DIPLOMA_TEST_2.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
double R;//a = c - little semi-axes 
double H;//b - big semi-axes
...
#pragma omp parallel for
                for (z = MIN_size; z < MAX_size; z += 1) {
                    if ((fabs(((pow((x - particle.x), 2) / pow(R, 2)) + (pow((y - particle.y), 2) / pow(R, 2))) - 1) < accur) && ((-H <= particle.z)||(particle.z <= H)) ) {
                        part_temp.x = x;
                        part_temp.y = y;
                        part_temp.z = z;
